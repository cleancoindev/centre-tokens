...
else initializing the roles and trying to use it as a token or pass it off as a real c-GAS token. 
   ```
   initialize(
          "",
          "",
...
1. Obtain addresses for the various contract roles from FCC/SS ops. The keys for these addresses will be stored offline.
